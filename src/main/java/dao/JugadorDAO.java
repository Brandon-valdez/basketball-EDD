...
public class JugadorDAO {
    ...
    public Jugador buscarPorId(int idJugador) throws SQLException {
        String sql = "SELECT j.*, e.nombre AS nombre_equipo FROM JUGADORES j JOIN EQUIPOS e ON e.id_equipo=j.id_equipo WHERE j.id_jugador=?";
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
    ...
    public boolean insertar(Jugador j) throws SQLException {
        String sql = "INSERT INTO JUGADORES (nombre, apellido, fecha_nac, posicion, numero_camiseta, id_equipo) VALUES (?,?,?,?,?,?)";
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
    ...
    public boolean actualizar(Jugador j) throws SQLException {
        String sql = "UPDATE JUGADORES SET nombre=?, apellido=?, fecha_nac=?, posicion=?, numero_camiseta=? WHERE id_jugador=?";
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
            ps.setString(1, j.getNombre());
            ps.setString(2, j.getApellido());
            ps.setDate(3, j.getFechaNac() != null ? Date.valueOf(j.getFechaNac()) : null);
    ...
    public boolean existeNumeroCamiseta(int numero, int idEquipo, int idJugadorExcluir) throws SQLException {
        String sql = "SELECT COUNT(*) FROM JUGADORES WHERE numero_camiseta=? AND id_equipo=? AND id_jugador<>?";
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
            ps.setInt(1, numero);
